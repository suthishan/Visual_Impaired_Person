...
public class BinaryShapeExtraction
{
	...
	static class ShapeIdentify
	{
		...
		public ShapeIdentify(int xMin, int xMax, int yMin, int yMax, int mass)
		{
			this.xMin = xMin;
			this.xMax = xMax;
			this.yMin = yMin;
	...
	public BinaryShapeExtraction(int width, int height)
	{
		this.width = width;
		this.height = height;
	...
	public byte[] detectBlobs(byte[] srcData, byte[] dstData, int minBlobMass, int maxBlobMass, byte matchVal, List<ShapeIdentify> ShapeList)
	{
		...
		if (ShapeList == null) ShapeList = new ArrayList<ShapeIdentify>();
		...
		for (int i=label-1 ; i>0 ; i--)
		{
			...
			else
			{
				...
				{
					ShapeIdentify ObjShape = new ShapeIdentify(xMinTable[i], xMaxTable[i], yMinTable[i], yMaxTable[i], massTable[i]);
